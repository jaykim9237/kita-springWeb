...
//목록 출력
//자바스크립트는 매개변수를 받을 때 타입을 지정해주지 않아도 알아서 알아 똑똑한 아이인걸? 오!
//그래서 지금 우리는 json(객체)를 msg로 받아왔어, 
function output(msg) {
	//서버로부터 받은 목록을 반복문으로 읽기
	//우리가 위에서 init으로 받은거잖아, 그 때 데이터타입(json)을 가지고 함수를 불렀잖아 그니까 이 함수가 아는거야!!
	var str = '<table>';
	//자바스크립트에서의 foreach함수의 형식
	//$.each(배열 function(한줄씩 뺄 열의 것들))------> 이건 그냥 왜워야해
